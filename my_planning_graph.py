...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # Node can only be reached if its preconditions are satisfied by previous state level
            # Test if prenodes  are a subset of the previous S level
            if node_a.prenodes.issubset(self.s_levels[level]):
                # Add this reachable action node to the list of action nodes
                # connecting the node to as child of preceding and preceding as parent of node
                a_nodes.append(node_a)
                for node_s in self.s_levels[level]:
                    node_s.children.add(node_a)
                    node_a.parents.add(node_s)
        # Build the action level
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level - 1]:
            # Get S nodes in effnodes that represent the effect produced by the action
            for node_s in node_a.effnodes:
                ...
                #connect all of the new S nodes as children of all the A nodes that could produce them
